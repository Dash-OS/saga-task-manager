...
import CreateSagaPromise from './utils/createSagaPromise'
...
class SagaTaskMan {
  ...
  * create(category, id, fn, ...args) {
    ...
    if ( ! category || ! id || ! fn ) {
      return this.handleError(`Tasks must have a category, id, and fn at a minimum but received ${category}.${id} - ${fn}`, 'critical')
    }
    try {
      if ( this.config.overwrite ) {
        yield call([this, this.cancel], category, id)
      } else if ( yield call([ this, this.taskExists ], category, id) ) {
        return this.handleError(`When overwrite config is set to false, you must cancel tasks before scheduling them again.  While Creating Task: ${category}.${id}`)
      }
      ...
      yield spawn(
        [ this, this.onTaskComplete ],
        category, id, [this, handleTaskCleanup]
      )
      ...
      this.handleError(`Failed to create task: (${category}.${id}): ${e.message}`)
      throw e
    }
  }
  ...
  * cancelTask(tasks, category, id, task) {
    if ( ! task && ! tasks.has(id) ) { return }
    task = task || tasks.get(id)
    if ( task && task[TASK] && task.isRunning() ) { yield cancel(task) }
    tasks.delete(id)
    if ( tasks.size === 0 ) { this.tasks.delete(category) }
    return task
  }
  ...
  * taskExists(category, id) {
  ...
  saveTask = (category, id, task, taskArgs) => {
    ...
    if ( categoryTasks.has(id) ) {
      this.handleError(`Failed to Save Task, ${category}.${id} already exists`, 'critical')
    } else {
      categoryTasks.set(id, task)
    }
  }

  handleCategoryComplete = category => this.handlers.resolvers.get(`onCategory.${category}`).resolve()

  handleError = (msg, level = 'error') => {
    this.handleLog('error', msg)
    if ( this.config.silent === true && level !== 'critical' ) { return }
    if ( this.config.silent === 'critical' ) { return }
    throw new Error(`[${this.getName()}] ${msg}`)
  }

  createHandler = (handler, ...args) => this.handlers.promises.set(
    handler,
    CreateSagaPromise(
      Promise,
      ((resolve, reject) =>
  ...
  kill = () => {
    if ( this.killed ) {
      this.handleLog('warn', 'Process is already killed but you called kill on it again!')
      return
    }
    this.killed = true

    const kill_promise = this.handlers.promises.get('onKilled')
      .then(r => {
        if ( ! this.deleted ) {
          TaskManagers.delete(this.id)
          delete window.tman[this.id]
        }
        return r
      })
      .catch(err => {
        this.handleError(`Failed to Kill Task Manager: ${err.message}`, 'critical')
        if ( ! this.deleted ) {
          TaskManagers.delete(this.id)
          delete window.tman[this.id]
        }
